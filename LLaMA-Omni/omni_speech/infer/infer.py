...
class CustomDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        noise = item["noise"]
        masked_noise = item['masked_noise']
        ...
        if sr != 16000:
            speech = librosa.resample(speech.numpy(), orig_sr=sr, target_sr=16000)
            speech = torch.from_numpy(speech)

        speech_length = speech.shape[1]
        masked_noise[:, :speech_length] = 1
        ...
        if self.input_type == "raw":
            ...
            clean_speech = speech
            adv_speech = speech + noise * masked_noise
            speech = adv_speech
            speech = whisper.log_mel_spectrogram(speech, n_mels=self.mel_size).permute(0, 2, 1)

        input_ids = tokenizer_speech_token(prompt, self.tokenizer, return_tensors='pt')

        return input_ids, clean_speech, adv_speech, speech, torch.LongTensor([speech_length]), torch.LongTensor([speech.shape[1]])
...
def collate_fn(batch):
    input_ids, clean_speech_tensors, adv_speech_tensor, speech_tensors, clean_speech_lengths, speech_lengths = zip(*batch)
    input_ids = torch.stack(input_ids, dim=0)
    speech_tensors = torch.stack(speech_tensors, dim=0)
    clean_speech_tensors = torch.stack(clean_speech_tensors, dim=0)
    clean_speech_lengths = torch.stack(speech_lengths, dim=0)
    speech_lengths = torch.stack(speech_lengths, dim=0)
    return input_ids, clean_speech_tensors, adv_speech_tensor, speech_tensors, clean_speech_lengths, speech_lengths
